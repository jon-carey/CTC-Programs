################################################################################################
# This code will summarize Agency (where available) and Model based forecasts for a 
# given set of years (beginning with 'StartYear' defined below), in addition to 
# actual observed returns. The data come from a set of CheckCLB.out files, with 
# forecasts coming out of files for each individual year, and actual returns all
# coming out of the most recent year's file.
#
# REQUIRED INPUTS:
# 1. CheckCLB.out files for all desired years to be included in summary
# 2. A csv file that provides a list of names (in chronological order ending with most recent)
#    of each CheckCLB file (since they aren't always named consistently); see 'filelist' below
# 3. A csv file that provides geographic (North -> South) ordering of stocks to be used in
#    the second set of figures; see 'StkGeoOrder' below
# 4. An output file path; see 'Outfile' below
#
# OUTPUTS:
