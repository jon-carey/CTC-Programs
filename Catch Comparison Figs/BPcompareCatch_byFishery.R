...
Fish_T <- FishLUT[FishLUT$TerminalID == 1, ]

# # TOTAL TERMINAL CATCH FIGURE
# 
# NewFishIDs <- unique(Fish_T$FishID_New)
# OldFishIDs <- na.omit((unique(Fish_T$FishID_Old)))
# 
# NewCatch <- Cat_NewBP[Cat_NewBP$FisheryNum %in% NewFishIDs, c(1,4:6)]
# NewCatch <- summaryBy(Observed_Catch + Model_LCatch ~ Year, data = NewCatch, FUN = sum)
# colnames(NewCatch) <- c("Year", "Observed_New", "Model_New")
...
# 
# q[[1]] <- ggplot(data = Catch, aes(Year, Catch, color = Source))
# q[[1]] <- q[[1]] + geom_line(size = 1.0)
# q[[1]] <- q[[1]] + scale_color_manual(values=c("#0072B2", "#D55E00", "#000001", "#999999"))
# q[[1]] <- q[[1]] + ggtitle(title)
# q[[1]] <- q[[1]] + theme(legend.position = "bottom")
# q[[1]] <- q[[1]] + scale_x_continuous(breaks = seq(from = 1980, to = 2016, by = 2))
# q[[1]] <- q[[1]] + scale_y_continuous(labels = comma)
# q[[1]] <- q[[1]] + theme(axis.text.x = element_text(angle = 45, vjust = 0.5))
# 
# # Save
# if(CreateJPEG == 1) {
#     ggsave(paste(Outfile,title,".jpg",sep=""),q[[1]],height=5,width=7.5)
# }
# 
# # Append data to SummaryTable
# Catch_wide$Fishery <- rep(title, times = dim(Catch_wide)[1])
...
# INDIVIDUAL TERMINAL FISHERY FIGURES 
i=1
while(i <= dim(Fish_T)[1]) {
    Fish_T_i <- Fish_T[i, ]
    OldFish <- subset(Fish_T, Fish_T$FishID_Old == Fish_T_i[1,2])
    
    title <- Fish_T[i,6]
    
    # This loop if for fisheries with exactly one ounterpart in each base period
    if(dim(OldFish)[1] == 1) {
        NewCatch <- Cat_NewBP[Cat_NewBP$FisheryNum == Fish_T$FishID_New[i], c(1,4:6)]
        colnames(NewCatch) <- c("Fishery", "Year", "Observed_New", "Model_New")
        
        OldCatch <- Cat_OldBP[Cat_OldBP$FisheryNum == Fish_T$FishID_Old[i], c(1,4:6)]
        colnames(OldCatch) <- c("Fishery", "Year", "Observed_Old", "Model_Old")
    ...
    # This loop is for new BP fisheries that don't have old BP counterparts
    if(dim(OldFish)[1] < 1) {
        NewCatch <- Cat_NewBP[Cat_NewBP$FisheryNum == Fish_T$FishID_New[i], c(4:6)]
        colnames(NewCatch) <- c("Year", "Observed_New", "Model_New")
    ...
    if(dim(OldFish)[1] >= 1) {
        q[[length(q)+1]] <- ggplot(data = Catch, aes(Year, Catch, color = Source))
        q[[length(q)]] <- q[[length(q)]] + geom_line(size = 1.0)
        q[[length(q)]] <- q[[length(q)]] + scale_color_manual(values=c("#0072B2", "#D55E00", "#000001", "#999999"))
        q[[length(q)]] <- q[[length(q)]] + ggtitle(title)
        q[[length(q)]] <- q[[length(q)]] + theme(legend.position = "bottom")
        q[[length(q)]] <- q[[length(q)]] + scale_x_continuous(breaks = seq(from = 1980, to = 2016, by = 2))
        q[[length(q)]] <- q[[length(q)]] + scale_y_continuous(labels = comma)
        q[[length(q)]] <- q[[length(q)]] + theme(axis.text.x = element_text(angle = 45, vjust = 0.5))
    }
    if(dim(OldFish)[1] < 1) {
        q[[length(q)+1]] <- ggplot(data = Catch, aes(Year, Catch, color = Source))
        q[[length(q)]] <- q[[length(q)]] + geom_line(size = 1.0)
        q[[length(q)]] <- q[[length(q)]] + scale_color_manual(values=c("#0072B2", "#000001"))
        q[[length(q)]] <- q[[length(q)]] + ggtitle(title)
        q[[length(q)]] <- q[[length(q)]] + theme(legend.position = "bottom")
        q[[length(q)]] <- q[[length(q)]] + scale_x_continuous(breaks = seq(from = 1980, to = 2016, by = 2))
        q[[length(q)]] <- q[[length(q)]] + scale_y_continuous(labels = comma)
    ...
    # Save
    if(CreateJPEG == 1) {
        ggsave(paste(Outfile,title,".jpg",sep=""),q[[length(q)]],height=5,width=7.5)
    }
...
if(CreatePDF == 1) {
    pdf(file=paste(Outfile,"T_Fishery_Catch_Comparisons.pdf",sep=""),height=7,width=10)
